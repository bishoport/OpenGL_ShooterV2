...
//        void DrawDebugLight(const std::string& shader)
//        {
//            UpdateVertices(); // Asegúrate de actualizar los vértices antes de dibujar
//
//            glm::mat4 model = glm::translate(glm::mat4(1.0f), transform.position);
//
//            libCore::ShaderManager::Get(shader)->setVec4("u_Color", glm::vec4(color, 1.0f));
...
//                for (const auto& point : areaLightpoints)
//                {
//                    vertices.push_back(transform.position + point);
//                }
//            }
//
//            glBindBuffer(GL_ARRAY_BUFFER, VBO);
...
//        Light(LightType lightType) : type(lightType), color(1.0f, 1.0f, 1.0f), intensity(10.0f)
//        {
//            // Inicializa puntos con un cuadrado por defecto y suma transform.position
//            areaLightpoints = {
//                glm::vec3(-1.0f, 0.0f, -1.0f),
//                glm::vec3(1.0f, 0.0f, -1.0f),
//                glm::vec3(1.0f, 0.0f, 1.0f),
...
//                {
//                    glm::vec3 transformedPoint = transform.position + areaLightpoints[i];
//                    libCore::ShaderManager::Get(shader)->setVec3("areaLights[" + std::to_string(id) + "].points[" + std::to_string(i) + "]", areaLightpoints[i]);
//                }
//                libCore::ShaderManager::Get(shader)->setBool("areaLights[" + std::to_string(id) + "].twoSided", twoSided);
//            }
//        }
